...
module.exports = class AliasUtils {
  ...
  //i stands for input
  create = async (i_name: string, i_link: string) =>
  ...
  find = async (i_name: string) => {
    ...
    if (search) return search.getDataValue('link');
  };

  //you only redefine the link, but i need the this.model to find the row
  redefine = async (i_name: string, i_link: string) => {
    const affected_rows = await this.model.update(
      { link: i_link },
      { where: { name: i_name } }
    );

    if (affected_rows <= 0) throw Error('Couldn`t find wanted row');
  };
